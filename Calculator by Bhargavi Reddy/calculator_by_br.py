...
def cal():
    root = Tk()
    root.title("Calculator by Bhargavi Reddy")
    root.resizable(0, 0)

    entry_font = font.Font(size=40)
    entry = Entry(root, justify="right", font=entry_font)
    entry.grid(row=0, column=0, columnspan=4,
               sticky=N + W + S + E, padx=50, pady=50)

    cal_button_bg = '#4b4f7d'
    num_button_bg = '#4B4B4B'
    other_button_bg = '#7da5b5'
    text_fg = '#bbc91e'
    button_active_bg = '#3499ba'

    num_button = partial(Button, root, fg=text_fg, bg=num_button_bg,
                         padx=30, pady=15, activebackground=button_active_bg)
    ...
    button14 = Button(root, text='/', fg=text_fg, bg=cal_button_bg, padx=30, pady=15,
                      command=lambda: get_input(entry, '/'))
    button14.grid(row=1, column=3, pady=5)

    button15 = Button(root, text=' <-- Backspace --> ', bg=other_button_bg, padx=30, pady=15,
    ...
    button16 = Button(root, text='Clear all', bg=other_button_bg, padx=30, pady=15,
                      command=lambda: clear(entry), activebackground=button_active_bg)
    button16.grid(row=1, column=2, pady=5)

    button17 = Button(root, text='=', fg=text_fg, bg=cal_button_bg, padx=30, pady=15,
                      command=lambda: calc(entry), activebackground=button_active_bg)
    button17.grid(row=5, column=3, pady=5)

    button18 = Button(root, text='^', fg=text_fg, bg=cal_button_bg, padx=30, pady=15,
                      command=lambda: get_input(entry, '**'))
    button18.grid(row=5, column=2, pady=5)
    def quit():
        exit['command'] = root.quit()
    exit = Button(root, text='Quit', fg='white', bg='black', command=quit, height=5, width=20)
